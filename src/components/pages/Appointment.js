        ...
        <AnimateHeight id="animForm" duration={300} animateOpacity={true} height={height}>
          <ContainerRowBtwn clearBtn={clearBtn} form='Appointment' showLoader={showLoader}  >
            ...
            <FormInnerRightPane onSubmitHandler={onSubmitHandler}>
              ...
              <DropDownInput handle={(e) => setCustomer(e.target.value)} name='Patient' label='Patient' >
                {customers.map((customer) => (
                  <option value={customer.id} val={customer.id} key={customer.id}> {customer.name} {customer.surname}  </option>
                ))}
              </DropDownInput>
