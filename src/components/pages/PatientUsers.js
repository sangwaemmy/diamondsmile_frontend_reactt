...
function PatientUsers() {
    ...
    const delAccountById = (id) => {
        Utils.Submit(() => {
            Delete.deleteAccountById(id, () => { getAllPatientUsers() })
        }, () => { })
    }
    /*#endregion Listing data*/
    ...
    return (
        <PagesWapper>
            ...
            <ContainerRow mt='3'>
                <ListToolBar listTitle='Accounts List' height={height} entity=' user' changeFormHeightClick={() => setHeight(height === 0 ? 'auto' : 0)} changeSearchheight={() => setSearchHeight(searchHeight === 0 ? 'auto' : 0)} handlePrint={handlePrint} searchHeight={searchHeight} />
