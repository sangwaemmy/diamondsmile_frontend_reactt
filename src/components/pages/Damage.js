...
function Damage() {
  ...
  const onSubmitHandler = (e) => {
    ...
    if (id) {
      ...
      Commons.saveDamage(tbl_sales, localStorage.getItem('userid'), items, reference).then((res) => {
  ...
  const getAllHw_movements = () => {
    Repository.findWhmByType('damage').then((res) => {
  ...
  const saleSwitchToDamage = () => {
    return (userType == 'store keeper' || userType == 'admin') ? 'Damage' : 'sale'
  }
                ...
                return (
                  <tr key={Hw_movement.id}>
                    ...
                    <td style={{ backgroundColor: Hw_movement.in_out == 'damage' ? '#ffd4c5e8' : 'transparent' }} >
                      {Hw_movement.in_out === 'in' ? 'Purchase' : (Hw_movement.in_out == 'damage' ? 'Damage' : 'sale')}   </td>
